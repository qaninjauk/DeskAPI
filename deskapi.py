...
def get_cases_by_label(label_name):
    ...
    r = requests.get(cf_site + '/api/v2/cases/search?sort_field=updated_at&per_page=500&since_updated_at=1386966252', headers=json_header, params=search_params, auth=(cf_login, cf_password))
    # r = requests.get(cf_site + '/api/v2/cases/search?since_created_at=1546300800&per_page=100&since_updated_at=1542074420', headers=json_header, params=search_params, auth=(cf_login, cf_password))
